...
private:
    ...
    int size;
public:
    DynamicArray() { //пустой масив
        this->size = 0;
        this->items = nullptr;
    }

    DynamicArray(T* items, int size) { //заполнение
        this->items = items;
    ...
    DynamicArray(int size) {//без значений
        this->items = new T[size];
        this->size = size;
    }

    DynamicArray(const DynamicArray <T>& dynamicArray) {//копирование
    ...
    void Set(int index, T value) {// исключения
        if (index < 0 || index >= this->size) throw out_of_range(IndexOutOfRangeEx);
    ...
    void delete_i(int index) {// удаляем элемент по индексу
        if (index < 0 || index >= this->size) throw out_of_range(IndexOutOfRangeEx);
